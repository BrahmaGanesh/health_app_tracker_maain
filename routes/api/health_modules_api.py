...
from models_new_modules import (
    LabTest, DoctorVisit, Appointment, EmergencyCard,
    TrustedContact, UserSubscription,
    HealthTimelineEvent,
)
...
def lab_tests():
    ...
    db.session.add(HealthTimelineEvent(
        user_id=uid, event_type="lab",
        event_date=test.test_date,
        title=f"🧪 {test.test_type.replace('_',' ').title()}: {test.value} {test.unit or ''}",
        description=f"Lab: {test.lab_name or 'Unknown'} · Status: {test.status}",
        icon="🧪", member_id=test.member_id,
    ))
    db.session.commit()
    return jsonify({"success": True, "message": "Lab test saved", "data": test.to_dict()}), 201
...
def doctor_visits():
    ...
    visit = DoctorVisit(
        ...
        follow_up_date=_parse_date(d.get("follow_up_date")),
        cost=d.get("cost"),
        notes=d.get("notes", "").strip() or None,
        member_id=d.get("member_id"),
    )
    ...
    db.session.add(HealthTimelineEvent(
        ...
        icon="👨‍⚕️", member_id=visit.member_id,
    ))
    db.session.commit()
    ...
    return jsonify({"success": True, "message": "Visit saved", "data": visit.to_dict()}), 201
...
def appointments():
    ...
    if request.method == "GET":
        ...
        if member_id:
            q = q.filter_by(member_id=member_id)
        if upcoming_only:
            q = q.filter(Appointment.appointment_date >= today_ist(), Appointment.completed == False)
    ...
    db.session.add(appt)
    db.session.commit()
    return jsonify({"success": True, "message": "📅 Appointment saved", "data": appt.to_dict()}), 201
...
def emergency_card():
    uid = get_jwt_identity()
    from models import User, Medicine as OldMedicine

    user = User.query.get(uid)
    card = EmergencyCard.query.filter_by(user_id=uid).first()

    if request.method == "GET":
        conditions = [c.condition.name for c in user.conditions] if user else []
        medicines  = [m.name for m in Medicine.query.filter_by(user_id=uid, is_active=True).limit(10).all()]
        return jsonify({"success": True, "data": {
            **(card.to_dict(user=user, conditions=conditions, medicines=medicines) if card
               else {"blood_group": None, "allergies": None, "emergency_contacts": None}),
...
# ════════════════════════════════════════════════════════════════
# MODULE 15 — TRUSTED CONTACTS (Emergency Alerts)
# ════════════════════════════════════════════════════════════════
...
def trusted_contacts():
    ...
    db.session.add(c)
    db.session.commit()
    return jsonify({"success": True, "message": f"Contact {c.name} added", "data": c.to_dict()}), 201
...
@modules_api_bp.route("/habits/", methods=["GET", "POST"])
@jwt_required()
def habits():
    uid = get_jwt_identity()
    ...
    if request.method == "GET":
        member_id = request.args.get("member_id", type=int)
        q = Habit.query.filter_by(user_id=uid, is_active=True)
        if member_id:
            q = q.filter_by(member_id=member_id)
        h_list = q.all()
        return jsonify({"success": True, "data": {
            "habits": [h.to_dict() for h in h_list],
            "completed_today": sum(1 for h in h_list if h.to_dict()["done_today"]),
        }})

    d = request.get_json() or {}
    if not d.get("name"):
        return jsonify({"success": False, "message": "Habit name required"}), 400
    ...
    db.session.commit()
    return jsonify({"success": True, "message": f"Habit '{habit.name}' created", "data": habit.to_dict()}), 201


@modules_api_bp.route("/habits/<int:hid>/log", methods=["POST"])
@jwt_required()
def log_habit(hid):
    uid   = get_jwt_identity()
    habit = Habit.query.filter_by(id=hid, user_id=uid).first_or_404()
    d     = request.get_json() or {}
    today = today_ist()

    log = HabitLog.query.filter_by(habit_id=hid, log_date=today).first()
    if log:
        log.completed    = bool(d.get("completed", True))
        log.actual_value = d.get("actual_value")
        log.logged_at    = now_ist()
    else:
        log = HabitLog(habit_id=hid, log_date=today,
                       completed=bool(d.get("completed", True)),
                       actual_value=d.get("actual_value"),
                       logged_at=now_ist())
        db.session.add(log)
    db.session.commit()
    return jsonify({"success": True, "message": f"{'✅' if log.completed else '⏭'} {habit.name}", "data": habit.to_dict()})


@modules_api_bp.route("/habits/<int:hid>", methods=["DELETE"])
@jwt_required()
def delete_habit(hid):
    ...
    db.session.commit()
    return jsonify({"success": True, "message": "Habit removed"})
...
def timeline():
    ...
    return jsonify({"success": True, "data": {
        ...
        "total": events.total, "page": page, "pages": events.pages,
    }})
...
def ai_chat():
    """
    Uses Anthropic Claude API for health-focused wellness chat.
    Keeps conversation context from history.
    """
    uid     = get_jwt_identity()
    d       = request.get_json() or {}
...
3. Keep responses concise and practical (2-4 sentences typically)
...
5. You know about BP management, diabetes, weight, sleep, and exercise"""
        ...
        reply = ("I'm unable to connect to the AI service right now. "
...
# ════════════════════════════════════════════════════════════════
# MODULE 6 — AI CAMERA (Food & Medicine Analysis)
# ════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            messages=[{"role": "user", "content": [
                ...
                {"type": "text", "text": "Identify the food and estimate: calories, protein, carbs, fats, sodium. Return JSON: {food_name, calories, protein_g, carbs_g, fat_g, sodium_mg, is_healthy_for_bp (true/false), notes}"},
            ]}],
        )
        import json
...
def analyze_medicine():
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            messages=[{"role": "user", "content": [
                {"type": "image", "source": {"type": "base64", "media_type": "image/jpeg", "data": image}},
                {"type": "text", "text": "Identify this medicine if visible. Return JSON: {medicine_name, active_ingredient, common_uses, typical_dosage, important_warnings, requires_prescription (true/false)}"},
            ]}],
        )
        import json, re
        text  = resp.content[0].text
...
def verify_purchase():
    ...
    # For now, update plan based on product_id
    plan_map = {
    ...
    return jsonify({"success": True, "message": f"🎉 Upgraded to {plan.title()}!", "data": sub.to_dict()})
...
def health_score_history():
    ...
    return jsonify({"success": True, "data": {
        "scores": [{"date": str(s.date), "score": s.total_score, "grade": s.grade} for s in scores],
    }})
...
def _auto_create_followup(user_id, visit):
    """Auto-create follow-up appointment from doctor visit."""
