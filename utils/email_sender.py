...
def run_scheduled_reports():
    ...
    with app.app_context():
        ...
        configs = EmailReportConfig.query.filter_by(is_enabled=True).all()
        now = datetime.utcnow()
        for config in configs:
            try:
                if config.next_send_at and now >= config.next_send_at:
                    send_report_for_user(config.user)
            except Exception as e:
                logger.error(f"Email report error user {config.user_id}: {e}")
...
def send_report_for_user(user, period_days=None, recipients=None):
    ...
    try:
        # ── Use Gmail API if user logged in with Google ───────────
        # This sends the report FROM the user's own Gmail address!
        gmail_sent = False
        ...
        # ── Fallback: standard SMTP (Flask-Mail) ─────────────────
        if not gmail_sent:
            msg = Message(
                subject    = subject,
        ...
        log = EmailReportLog(
            user_id=user.id, status="failed", error_message=str(e)
        )
        db.session.add(log)
        db.session.commit()
...
def _compute_next_send(config):
    ...
    if config.frequency == "daily":
        ...
        return now + timedelta(days=config.custom_days or 7)
    return now + timedelta(weeks=1)