...
def format_ist_time(dt):
    if dt is None:
        return ""
    return to_ist_dt(dt).strftime("%I:%M %p")


def format_ist_date(dt):
    if dt is None:
        return ""
    return to_ist_dt(dt).strftime("%d %b %Y")
...
class User(UserMixin, db.Model):
    ...
    family_member_limit = db.Column(db.Integer, default=2)
    ...
    medicines = db.relationship("Medicine", backref="user", cascade="all, delete-orphan", lazy="dynamic")
    ...
    habit_logs = db.relationship("HabitLogLegacy", backref="user", cascade="all, delete-orphan", lazy="dynamic")
    ...
    lab_tests = db.relationship("LabTest", backref="user", cascade="all, delete-orphan", lazy="dynamic")
    doctor_visits = db.relationship("DoctorVisit", backref="user", cascade="all, delete-orphan", lazy="dynamic")
    appointments = db.relationship("Appointment", backref="user", cascade="all, delete-orphan", lazy="dynamic")
    emergency_card = db.relationship("EmergencyCard", backref="user", uselist=False, cascade="all, delete-orphan")
    trusted_contacts = db.relationship("TrustedContact", backref="user", cascade="all, delete-orphan", lazy="dynamic")
    subscription = db.relationship("UserSubscription", backref="user", uselist=False, cascade="all, delete-orphan")
    timeline_events = db.relationship("HealthTimelineEvent", backref="user", cascade="all, delete-orphan", lazy="dynamic")
...
class UserHealthProfile(db.Model):
    __tablename__ = "userhealthprofiles"
...
class UserCondition(db.Model):
    ...
    condition_id = db.Column(db.Integer, db.ForeignKey("healthconditions.id"), nullable=False)
...
class HealthMetric(db.Model):
    ...
    def bp_status(self):
        if self.metric_type != "bp":
            return None
        s, d = self.value1 or 0, self.value2 or 0
        if s < 120 and d < 80:
            return "Normal"
        elif s < 130 and d < 80:
            return "Elevated"
    ...
    def to_dict(self):
        ...
        return {
            ...
            "recorded_time": dt.strftime("%I:%M %p") if dt else "",
            "recorded_date": dt.strftime("%d %b %Y") if dt else "",
            "recorded_datetime_ist": dt.strftime("%d %b %Y, %I:%M %p IST") if dt else "",
            "bp_status": self.bp_status,
        }


class SleepLog(db.Model):
    __tablename__ = "sleeplogs"
    ...
    notes = db.Column(db.Text, nullable=True)
    recorded_at = db.Column(db.DateTime, default=now_ist)
...
class StepLog(db.Model):
    ...
    def compute_distance(self, height_cm=170):
        if not self.steps:
            return 0
        return round((self.steps * height_cm * 0.414) / 100000, 2)
...
class FamilyMember(db.Model):
    ...
    lab_tests = db.relationship("LabTest", backref="member", cascade="all, delete-orphan", lazy="dynamic")
    doctor_visits = db.relationship("DoctorVisit", backref="member", cascade="all, delete-orphan", lazy="dynamic")
    appointments = db.relationship("Appointment", backref="member", cascade="all, delete-orphan", lazy="dynamic")
    timeline_events = db.relationship("HealthTimelineEvent", backref="member", cascade="all, delete-orphan", lazy="dynamic")
    habits = db.relationship("Habit", backref="member", cascade="all, delete-orphan", lazy="dynamic")
...
class FamilyHealthMetric(db.Model):
    __tablename__ = "familyhealthmetrics"

    id = db.Column(db.Integer, primary_key=True)
    member_id = db.Column(db.Integer, db.ForeignKey("familymembers.id"), nullable=False, index=True)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "value2": self.value2,
            "recorded_at": dt.isoformat() if dt else None,
            "recorded_time": dt.strftime("%I:%M %p IST") if dt else "",
            "recorded_date": dt.strftime("%d %b %Y") if dt else "",
        }


class FamilyMedicine(db.Model):
    __tablename__ = "familymedicines"

    id = db.Column(db.Integer, primary_key=True)
    member_id = db.Column(db.Integer, db.ForeignKey("familymembers.id"), nullable=False)
...
class Medicine(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False, index=True)
    member_id = db.Column(db.Integer, db.ForeignKey("familymembers.id"), nullable=True)

    name = db.Column(db.String(200), nullable=False)
    generic_name = db.Column(db.String(200), nullable=True)
    dosage = db.Column(db.String(100), nullable=True)
    medicine_type = db.Column(db.String(50), default="tablet")
    with_food = db.Column(db.String(30), default="after_food")
    condition_name = db.Column(db.String(200), nullable=True)
    prescribed_by = db.Column(db.String(200), nullable=True)
    notes = db.Column(db.Text, nullable=True)

    schedule_type = db.Column(db.String(30), default="morning")
    ...
    auto_reduce_stock = db.Column(db.Boolean, default=True)

    reminder_enabled = db.Column(db.Boolean, default=True)
    snooze_minutes = db.Column(db.Integer, default=10)
    max_reminders = db.Column(db.Integer, default=3)
    reminder_sound = db.Column(db.String(50), default="medicine")

    caregiver_alert = db.Column(db.Boolean, default=False)
    ...
    PLAN_LIMITS = {"free": 2, "normal": 2, "premium": 10, "family": 20}
    ...
    def custom_times(self):
        try:
            return json.loads(self.custom_times_json or "[]")
        except Exception:
            return []
    ...
    def reminder_times(self):
        if self.schedule_type == "custom":
            return self.custom_times
        return [self.SCHEDULE_TIMES.get(self.schedule_type, "08:00")]

    def _is_taken_today(self):
        t = today_ist()
        log = self.logs.filter_by(log_date=t).first()
    ...
    def _adherence_30d(self):
        ...
        if not logs:
            return 0.0
        taken = sum(1 for l in logs if l.status in ("taken", "ai_verified"))
    ...
    def to_dict(self):
        return {
            ...
            "type_label": self.TYPES.get(self.medicine_type, "Tablet"),
            ...
            "auto_reduce_stock": self.auto_reduce_stock,
            "low_stock_alert": self.stock_count <= self.low_stock_threshold,
            "reminder_enabled": self.reminder_enabled,
            "snooze_minutes": self.snooze_minutes,
            "max_reminders": self.max_reminders,
            "reminder_sound": self.reminder_sound,
            "caregiver_alert": self.caregiver_alert,
            "today_status": self._is_taken_today(),
            "adherence_pct": self._adherence_30d(),
...
class MedicineLog(db.Model):
    ...
    log_date = db.Column(db.Date, nullable=False, default=today_ist, index=True)
    ...
    __table_args__ = (db.UniqueConstraint("medicine_id", "log_date", "scheduled_time"),)
    ...
    def to_dict(self):
        return {
            ...
            "log_date": str(self.log_date),
            "status": self.status,
            "status_emoji": self.STATUS_EMOJI.get(self.status, "🟡"),
...
class LabTest(db.Model):
    ...
    test_category = db.Column(db.String(50), nullable=False, default="blood_sugar")
    ...
    ref_range_label = db.Column(db.String(100), nullable=True)

    test_date = db.Column(db.Date, nullable=False, default=today_ist, index=True)
    lab_name = db.Column(db.String(200), nullable=True)
    doctor_name = db.Column(db.String(200), nullable=True)
    notes = db.Column(db.Text, nullable=True)

    document_id = db.Column(db.Integer, db.ForeignKey("documents.id"), nullable=True)
    repeat_reminder_months = db.Column(db.Integer, nullable=True)

    created_at = db.Column(db.DateTime, default=now_ist)
    updated_at = db.Column(db.DateTime, default=now_ist, onupdate=now_ist)

    DEFAULT_RANGES = {
        "blood_sugar": [("Low", 0, 69), ("Normal", 70, 99), ("Pre-Diabetic", 100, 125), ("Diabetic", 126, 9999)],
        "hba1c": [("Normal", 0, 5.6), ("Pre-Diabetic", 5.7, 6.4), ("Diabetic", 6.5, 99)],
        "cholesterol": [("Normal", 0, 199), ("Borderline", 200, 239), ("High", 240, 9999)],
        "hdl": [("Low", 0, 39), ("Borderline", 40, 59), ("Good", 60, 999)],
        "ldl": [("Optimal", 0, 99), ("Near Optimal", 100, 129), ("Borderline", 130, 159), ("High", 160, 9999)],
        "triglycerides": [("Normal", 0, 149), ("Borderline", 150, 199), ("High", 200, 499), ("Very High", 500, 9999)],
        "hemoglobin": [("Low", 0, 11.9), ("Normal", 12, 17.5), ("High", 17.6, 99)],
        "creatinine": [("Low", 0, 0.59), ("Normal", 0.6, 1.2), ("High", 1.21, 99)],
        "alt": [("Normal", 0, 40), ("Elevated", 41, 120), ("High", 121, 9999)],
        "ast": [("Normal", 0, 40), ("Elevated", 41, 120), ("High", 121, 9999)],
        "tsh": [("Low", 0, 0.39), ("Normal", 0.4, 4.0), ("High", 4.01, 99)],
        "vitamin_d": [("Deficient", 0, 19), ("Insufficient", 20, 29), ("Sufficient", 30, 100), ("Toxic", 101, 9999)],
        "vitamin_b12": [("Deficient", 0, 199), ("Normal", 200, 900), ("High", 901, 9999)],
        "wbc": [("Low", 0, 3.9), ("Normal", 4.0, 11.0), ("High", 11.01, 99)],
        "rbc": [("Low", 0, 3.9), ("Normal", 4.0, 5.9), ("High", 6.0, 99)],
        "platelets": [("Low", 0, 149), ("Normal", 150, 400), ("High", 401, 9999)],
        "uric_acid": [("Low", 0, 2.4), ("Normal", 2.5, 6.0), ("High", 6.01, 99)],
    }
    ...
    @property
    def status(self):
        ranges = self.DEFAULT_RANGES.get(
            self.test_name.lower().replace(" ", "_"),
            self.DEFAULT_RANGES.get(self.test_category, []),
        )
        if self.ref_range_low is not None and self.ref_range_high is not None:
            if self.value < self.ref_range_low:
                return "Low"
            if self.value > self.ref_range_high:
                return "High"
            return "Normal"
    ...
    def is_abnormal(self):
        return self.status not in ("Normal", "Good", "Sufficient", "Optimal")

    def to_dict(self, include_linked_doc=False):
        d = {
            "id": self.id,
            ...
            "ref_range_label": self.ref_range_label,
            ...
            "repeat_reminder_months": self.repeat_reminder_months,
            "member_id": self.member_id,
            "created_at": self.created_at.isoformat() if self.created_at else None,
        }
        if include_linked_doc and self.document_id:
            doc = Document.query.get(self.document_id)
            if doc:
                d["linked_document"] = doc.to_dict()
        return d
...
class Appointment(db.Model):
    ...
    appointment_type = db.Column(db.String(50), default="doctor")
    doctor_name = db.Column(db.String(200), nullable=True)
    ...
    reminder_1day = db.Column(db.Boolean, default=True)
    reminder_1hour = db.Column(db.Boolean, default=True)
    reminder_at_time = db.Column(db.Boolean, default=True)
    reminder_custom_mins = db.Column(db.Integer, nullable=True)
    reminder_sent_1day = db.Column(db.Boolean, default=False)
    reminder_sent_1hour = db.Column(db.Boolean, default=False)
    reminder_email = db.Column(db.Boolean, default=False)

    created_at = db.Column(db.DateTime, default=now_ist)
    updated_at = db.Column(db.DateTime, default=now_ist, onupdate=now_ist)

    PLAN_LIMITS = {"free": 2, "normal": 2, "premium": None, "family": None}

    TYPES = {
        "doctor": ("👨‍⚕️", "Doctor Visit"),
        "lab": ("🧪", "Lab Test"),
    ...
    @property
    def type_icon(self):
        return self.TYPES.get(self.appointment_type, ("📅", "Other"))[0]

    @property
    def type_label(self):
        return self.TYPES.get(self.appointment_type, ("📅", "Other"))[1]
    ...
    def days_until(self):
        if self.status != "upcoming":
            return None
        return (self.appointment_date - today_ist()).days

    @property
    def is_today(self):
        return self.appointment_date == today_ist()

    @property
    def is_overdue(self):
        return self.status == "upcoming" and self.appointment_date < today_ist()
    ...
    def to_dict(self):
        return {
            ...
            "completed": self.completed,
            "reminder_1day": self.reminder_1day,
            "reminder_1hour": self.reminder_1hour,
            "reminder_at_time": self.reminder_at_time,
            "reminder_custom_mins": self.reminder_custom_mins,
...
# ============================================================
# EMERGENCY / TRUSTED CONTACTS / SUBSCRIPTION / TIMELINE
# ============================================================
...
class Habit(db.Model):
    ...
    def _streak(self):
        ...
        check = today_ist()
        for _ in range(365):
            log = self.logs.filter_by(log_date=check, completed=True).first()
            if log:
                streak += 1
            else:
                break
    ...
    def to_dict(self):
        today = today_ist()
        today_log = self.logs.filter_by(log_date=today).first()
        return {
            "id": self.id,
            "name": self.name,
            "category": self.category,
            ...
            "is_active": self.is_active,
            "member_id": self.member_id,
            "done_today": today_log.completed if today_log else False,
            "streak": self._streak(),
        }
...
class HabitLog(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("habit_id", "log_date"),)
...
class Reminder(db.Model):
    ...
    sound_name = db.Column(db.String(50), default="healthalert")
    is_active = db.Column(db.Boolean, default=True)
...
class Notification(db.Model):
    ...
    message = db.Column(db.Text, nullable=False)
    icon = db.Column(db.String(10), default="")
    sound = db.Column(db.String(50), default="healthalert")
    ...
    def to_dict(self):
        return {
            ...
            "sound": self.sound,
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "created_time": self.created_at.strftime("%I:%M %p") if self.created_at else "",
        }


# ============================================================
# FOOD / DOCS / REPORTING / ADMIN
# ============================================================
...
class Recipe(db.Model):
    ...
    def composite_score(self, conditions):
        ...
        if "weightloss" in w:
            s += (self.weight_loss_score or 0) * w["weightloss"] / t
        return round(s, 1)
...
class MealPlan(db.Model):
    ...
    items = db.relationship("MealItem", backref="mealplan", cascade="all, delete-orphan")
...
class MealItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    plan_id = db.Column(db.Integer, db.ForeignKey("mealplans.id"), nullable=False)
    recipe_id = db.Column(db.Integer, db.ForeignKey("recipes.id"), nullable=False)
...
class NutritionDailyLog(db.Model):
    __tablename__ = "nutritiondailylogs"
...
class Document(db.Model):
    ...
    PLAN_LIMITS = {"free": 3, "normal": 3, "premium": 10, "family": 20}
    CATEGORIES = {
    ...
    @property
    def type_icon(self):
        return self.CATEGORIES.get(self.doc_type, ("", "Other"))[0]

    @property
    def type_label(self):
        return self.CATEGORIES.get(self.doc_type, ("", "Other"))[1]

    @property
    def is_image(self):
        return self.mime_type in ("image/jpeg", "image/png", "image/jpg")
    ...
    def to_dict(self):
        return {
            ...
            "tags": self.tags,
            "uploaded_at": self.uploaded_at.strftime("%d %b %Y") if self.uploaded_at else None,
            "uploaded_ts": self.uploaded_at.isoformat() if self.uploaded_at else None,
        }


class EmailReportConfig(db.Model):
    __tablename__ = "emailreportconfigs"
...
class DailyHealthScore(db.Model):
    __tablename__ = "dailyhealthscores"
