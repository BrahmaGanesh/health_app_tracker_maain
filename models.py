...
# ── TIMEZONE SETUP ────────────────────────────────────────────────
# All times stored in IST (UTC+5:30) — India Standard Time
# Server logs show UTC which is normal — app data uses IST

IST = timezone(timedelta(hours=5, minutes=30))

def now_ist():
    """
    Returns current live datetime in IST (UTC+5:30).
    This is what gets stored in the DB when a record is created/updated.
    Example: if UTC is 17:24, IST = 22:54
    """
...
def now_utc():
    """Returns current UTC datetime (for HealthMetric.recorded_at only)."""
    return datetime.now(timezone.utc).replace(tzinfo=None)

def to_ist(dt):
    """Convert any UTC-naive datetime to IST for display."""
    if dt is None:
        return None
    if dt.tzinfo is None:
        # Assume stored as IST naive — return as-is for display
        return dt
    return dt.astimezone(IST).replace(tzinfo=None)

def format_ist_time(dt):
    """Format datetime to IST time string like '10:54 PM'."""
    if dt is None:
        return ''
    ist_dt = to_ist(dt)
...
def format_ist_date(dt):
    """Format datetime to IST date string like '27 Jun 2026'."""
    if dt is None:
        return ''
    ist_dt = to_ist(dt)
...
class User(UserMixin, db.Model):
    ...
    # FCM token for push notifications (APK)
    fcm_token           = db.Column(db.String(500), nullable=True)
    # Web push subscription (Website)
    ...
    # Stored to send email reports via Gmail on behalf of user
    ...
    @property
    def bmi(self):
        if not self.health_profile: return None
    ...
    def bmi_status(self):
        ...
        elif b < 35:  return "Obese I"
        elif b < 40:  return "Obese II"
        return "Obese III"
    ...
    def daily_protein_target(self):
        if not self.health_profile: return 80
        w = self.health_profile.current_weight_kg or 70
        if "High Blood Pressure" in self.condition_names: return int(w * 1.0)
        if any("Weight Loss" in n for n in self.condition_names): return int(w * 1.4)
        return int(w * 1.0)
    ...
    @property
    def has_bp(self):        return "High Blood Pressure" in self.condition_names
    @property
    def has_diabetes(self):  return any("Diabetes" in n for n in self.condition_names)
    @property
    def has_weight_loss(self): return any("Weight Loss" in n for n in self.condition_names)

    def to_dict(self):
        """API serialization for APK."""
        return {
            "id": self.id, "name": self.name, "email": self.email,
            "is_verified": self.is_verified, "dark_mode": self.dark_mode,
            "profile_photo": self.profile_photo,
            "bmi": self.bmi, "bmi_status": self.bmi_status, "age": self.age,
...
class HealthMetric(db.Model):
    ...
    # bp / weight / sugar / water / steps / sleep / exercise / mood / calories
    ...
    @property
    def bp_status(self):
        if self.metric_type != "bp": return None
    ...
    def to_dict(self):
        # recorded_at stored as IST naive datetime
        dt = self.recorded_at
        return {
            "id": self.id, "metric_type": self.metric_type,
            "value_1": self.value_1, "value_2": self.value_2, "value_3": self.value_3,
...
class SleepLog(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id, "log_date": str(self.log_date),
            "sleep_time": self.sleep_time, "wake_time": self.wake_time,
            "duration_hours": self.duration_hours, "quality": self.quality,
            "quality_label": self.quality_label, "mood_on_wake": self.mood_on_wake,
            "interruptions": self.interruptions, "notes": self.notes,
        }
...
class ExerciseLog(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id, "log_date": str(self.log_date),
            "exercise_name": self.exercise_name, "exercise_type": self.exercise_type,
            "duration_minutes": self.duration_minutes, "calories_burned": self.calories_burned,
            "sets": self.sets, "reps": self.reps, "distance_km": self.distance_km,
            "intensity": self.intensity, "recorded_at": self.recorded_at.isoformat(),
        }
...
class StepLog(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "calories_burned": self.calories_burned, "goal_steps": self.goal_steps,
...
class HeartRateLog(db.Model):
    ...
    def to_dict(self):
        return {"id": self.id, "bpm": self.bpm, "reading_type": self.reading_type,
                "log_date": str(self.log_date), "recorded_at": self.recorded_at.isoformat()}
...
class ExerciseLibrary(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "muscle_group": self.muscle_group, "difficulty": self.difficulty,
            "description": self.description, "instructions": self.instructions,
            "benefits": self.benefits, "image_url": self.image_url,
            "duration_mins": self.duration_mins, "calories_per_min": self.calories_per_min,
            "bp_safe": self.bp_safe, "equipment": self.equipment,
...
class FamilyMember(db.Model):
    ...
    @property
    def bmi(self):
        if not self.height_cm or not self.current_weight_kg: return None
        return round(self.current_weight_kg / ((self.height_cm / 100) ** 2), 1)

    def to_dict(self):
        return {
            "id": self.id, "name": self.name, "relation": self.relation,
            "gender": self.gender, "age": self.age, "bmi": self.bmi,
            "height_cm": self.height_cm, "current_weight_kg": self.current_weight_kg,
            "blood_group": self.blood_group, "conditions": self.conditions,
            "profile_photo": self.profile_photo,
        }
...
class FamilyHealthMetric(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            "id": self.id, "metric_type": self.metric_type,
            "value_1": self.value_1, "value_2": self.value_2,
            "recorded_at": dt.isoformat() if dt else None,
...
# ============================================================
# REMINDER SYSTEM (Smart Repeating)
# ============================================================

class Reminder(db.Model):
    """
    Smart repeating reminder system.
    Sends notification every N minutes until user marks done.
    """
    ...
    # water / medicine / bp / exercise / sleep / sugar / steps / custom

    # Schedule
    remind_time         = db.Column(db.String(10), nullable=True)   # "07:30" daily time
    is_daily            = db.Column(db.Boolean, default=True)
    active_days         = db.Column(db.String(50), default="1,2,3,4,5,6,7")
    # comma-separated: 1=Mon...7=Sun

    # Repeat interval UNTIL done
    repeat_interval_mins= db.Column(db.Integer, default=5)
    # 3 / 5 / 10 — resend every N mins until marked done

    max_repeats         = db.Column(db.Integer, default=10)
    # Max times to repeat (prevents infinite spam)

    sound_enabled       = db.Column(db.Boolean, default=True)
    sound_name          = db.Column(db.String(50), default="health_alert")
    # health_alert / gentle / urgent / water_drop / medicine
    ...
    def to_dict(self):
        return {
            ...
            "category": self.category, "remind_time": self.remind_time,
            "repeat_interval_mins": self.repeat_interval_mins,
            "sound_enabled": self.sound_enabled, "sound_name": self.sound_name,
            "is_active": self.is_active, "is_done_today": self.is_done_today,
...
class Notification(db.Model):
    ...
    # reminder / achievement / alert / system / report
    category        = db.Column(db.String(30), nullable=True)
    title           = db.Column(db.String(200), nullable=False)
    message         = db.Column(db.Text, nullable=False)
    icon            = db.Column(db.String(10), default="🔔")
    ...
    def to_dict(self):
        return {
            "id": self.id, "notif_type": self.notif_type, "category": self.category,
            "title": self.title, "message": self.message, "icon": self.icon,
            "is_read": self.is_read, "sound": self.sound,
...
class Recipe(db.Model):
    ...
    def composite_score(self, conditions):
        w = {}
        if "High Blood Pressure" in conditions:       w["bp"] = 0.40
        if any("Diabetes" in c for c in conditions):  w["diabetes"] = 0.35
        if any("Weight Loss" in c for c in conditions): w["weight_loss"] = 0.25
        if not w: return self.nutrition_score or 50
        t = sum(w.values())
        s = 0
        if "bp" in w:          s += (self.bp_score or 0) * (w["bp"] / t)
        if "diabetes" in w:    s += (self.diabetes_score or 0) * (w["diabetes"] / t)
        if "weight_loss" in w: s += (self.weight_loss_score or 0) * (w["weight_loss"] / t)
        return round(s, 1)

    def to_dict(self):
        return {
            "id": self.id, "name": self.name, "category": self.category,
            "is_veg": self.is_veg, "calories": self.calories, "protein": self.protein,
            "carbs": self.carbs, "fats": self.fats, "fiber": self.fiber,
            "sodium": self.sodium, "potassium": self.potassium,
            "ingredients": self.ingredients, "health_benefits": self.health_benefits,
            "bp_friendly": self.bp_friendly, "bp_score": self.bp_score,
...
class NutritionDailyLog(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "total_calories": self.total_calories, "total_protein": self.total_protein,
            "total_carbs": self.total_carbs, "total_fats": self.total_fats,
            "total_fiber": self.total_fiber, "total_sodium": self.total_sodium,
            "total_water": self.total_water, "health_score": self.health_score,
        }
...
class Alert(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id, "alert_type": self.alert_type, "category": self.category,
...
class Document(db.Model):
    ...
    # lab_report / prescription / insurance / xray / ecg / mri / vaccination / other
    ...
    @property
    def type_icon(self):
        return {"lab_report":"🧪","prescription":"💊","insurance":"🛡️","xray":"🦴",
                "ecg":"❤️","mri":"🧠","vaccination":"💉","other":"📄"}.get(self.doc_type,"📄")

    def to_dict(self):
        return {
            "id": self.id, "title": self.title, "doc_type": self.doc_type,
            "type_icon": self.type_icon, "file_name": self.file_name,
            "doctor_name": self.doctor_name, "hospital_name": self.hospital_name,
...
class EmailReportConfig(db.Model):
    ...
    def recipient_list(self):
        if not self.email_recipients: return []
...
class DailyHealthScore(db.Model):
    ...
    def grade_label(self):
        ...
        elif s >= 60: return "Fair"
        elif s >= 40: return "Needs Attention"
        return "Critical"
    ...
    def to_dict(self):
        return {
            ...
            "bp_score": self.bp_score, "water_score": self.water_score,
            "sleep_score": self.sleep_score, "exercise_score": self.exercise_score,
            "steps_score": self.steps_score, "medicine_score": self.medicine_score,
        }
